...
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile2`, function (sprite, location) {
    game.showLongText("Good Job, You passed the tutorial! Lets play Level 1!", DialogLayout.Center)
    ...
    tiles.placeOnRandomTile(cat, sprites.swamp.swampTile0)
})
scene.onOverlapTile(SpriteKind.Player, assets.tile`myTile5`, function (sprite, location) {
    game.over(false, effects.melt)
})
scene.onOverlapTile(SpriteKind.Player, sprites.dungeon.hazardSpike, function (sprite, location) {
    game.showLongText("Good Job! You unlocked Level 2! Also for making it this far without dying, we will give you 3 bonus hearts.", DialogLayout.Center)
    info.changeLifeBy(3)
})
sprites.onOverlap(SpriteKind.Player, SpriteKind.Enemy, function (sprite, otherSprite) {
    bee.destroy()
    if (mySprite.y < otherSprite.x) {
